...
fc <- function(file){
  df = file |> 
    ...
    filter(is.na(ensmbleID) == FALSE & padj < 0.1) 
...
# merge into a table, and add a column of cryptic events
rbp_table = tibble(experiment, gene_name, fc, padj) |> 
  left_join(tdp_experiment, by = c('experiment' = 'name')) |> 
