...
def read_bam(f, c, s):
        ...
        #for read in samfile.fetch("4:200000-230000", start, end):
...
def get_bam_path(index, path):
        print(path, os.path.isabs(path),os.path.realpath(path))
        if os.path.isabs(path):
                return os.path.realpath(path).replace("/SAN","/Users/alla/cluster")
        ...
        for strand in bam_dict:
                ...
                for (bam_index in 1:length(density_list)) {
                        ...
                        for (i in row_i) {
                                ...
                                if (j[1] == %(args.start)s+1 & j[2] == %(args.end)s ) { lwd = 10 }
